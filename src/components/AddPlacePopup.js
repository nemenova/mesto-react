...
function AddPlacePopup(props) {
    ...
    return (
        <PopupWithForm onSubmit={handleAddPlaceSubmit} name="add-card" title="Новое место" buttonName="Создать" isOpened={props.isOpened} onClose={props.onClose}>
            <fieldset className="form__input">
                <input id="place-name-input" name="place-name" placeholder="Название" type="text" minLength="2" maxLength="30"
                    onChange={handleAddPlaceName} value={place} className="form__item form__item_el_place-name" required></input>
                ...
                <input id="place-img-input" name="place-img" placeholder="Ссылка на картинку" type="url"
                    onChange={handleAddPlaceLink} value={link} className="form__item form__item_el_place-img" required></input>
